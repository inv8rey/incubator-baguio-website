...
export default function HomeStats() {
  ...
  const cells: { value: number; label: string }[] = [
    { value: stats.innovators, label: "Innovators" },
    { value: stats.ecosystemOrganizations, label: "Ecosystem organizations" },
    { value: stats.openChallenges, label: "Open challenges" },
    { value: stats.tbis, label: "TBIs" },
  ];
  ...
  return (
    <div style={{ background: "#100D0B", padding: "0 40px 72px" }}>
      <div style={{ maxWidth: 1120, margin: "0 auto", borderTop: "1px solid rgba(255,255,255,0.08)", display: "grid", gridTemplateColumns: "repeat(4,1fr)" }} className="ib-ecosystem-grid">
        {cells.map((c, i) => (
          <div key={c.label} style={{ textAlign: "center", padding: "44px 20px 4px", borderLeft: i > 0 ? "1px solid rgba(255,255,255,0.08)" : undefined }}>
