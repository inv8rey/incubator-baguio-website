            ...
            return (
              <div key={c.id} className="ib-card-hover" style={{ background: "#fff", border: "1px solid rgba(20,20,25,0.12)", borderRadius: 18, padding: 26, display: "flex", flexDirection: "column" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18, gap: 8 }}>
                  <span style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: "0.06em", color: cat.color, background: cat.bg, padding: "6px 13px", borderRadius: 9999 }}>{cat.emoji} {c.category}</span>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 7, fontSize: 13, fontWeight: 700, color: c.deadlineColor, whiteSpace: "nowrap" }}>
                    <span style={{ width: 7, height: 7, borderRadius: 9999, background: c.deadlineColor }} />
                    {c.deadline}
                  </span>
                </div>
                <h3 style={{ margin: "0 0 12px", fontSize: 20, fontWeight: 700, letterSpacing: "-0.02em", color: DARK, lineHeight: 1.2 }}>{c.title}</h3>
                <p style={{ margin: "0 0 20px", fontSize: 14, lineHeight: 1.5, color: "#6B6B73", flex: 1, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{c.summary}</p>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: "auto", paddingTop: 18, borderTop: "1px solid rgba(20,20,25,0.1)" }}>
                  <div style={{ width: 38, height: 38, borderRadius: 10, background: c.orgColor, display: "flex", alignItems: "center", justifyContent: "center", fontSize: c.orgInitialsFontSize, fontWeight: 700, color: "#fff", flexShrink: 0 }}>{c.orgInitials}</div>
                  <span style={{ fontSize: 13.5, color: "#9A958B", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.orgName}</span>
                  <a href={`${bp}/challenges/${c.id}/`} style={{ marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 6, fontSize: 14, fontWeight: 600, color: DARK, textDecoration: "none", whiteSpace: "nowrap" }}>
                    View details
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={DARK} strokeWidth={2.4}><path d="M5 12h14M13 6l6 6-6 6"></path></svg>
                  </a>
                </div>
              </div>
            );
