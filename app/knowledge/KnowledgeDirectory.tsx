      ...
      {filtered.length > 0 ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 24 }} className="ib-knowledge-grid">
          {filtered.map((r) => {
            ...
            return (
              <div key={r.id} className="ib-card-hover" style={{ position: "relative", background: "#fff", border: r.featured ? "1.5px solid rgba(242,101,34,0.35)" : "1px solid rgba(64,50,34,0.13)", borderRadius: 18, padding: 24, display: "flex", flexDirection: "column" }}>
                {r.featured && (
                ...
                {href && (
                  <a
                    ...
                    style={{ marginTop: 16, alignSelf: "flex-start", display: "inline-flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 600, color: "#fff", background: "#1A1714", textDecoration: "none", padding: "10px 18px", borderRadius: 9999 }}
                  >
