...
export default function CommunityChallengeDetail({ bp }: { bp: string }) {
  ...
  if (notFound) {
    return (
      <div style={{ background: "#F6F2EA", padding: "64px 40px", textAlign: "center" }}>
        <p style={{ fontSize: 14.5, color: "#5A544B" }}>Challenge not found.</p>
  ...
  return (
    <>
      <div style={{ position: "relative", background: "#100D0B", padding: "44px 40px 48px", overflow: "hidden", textAlign: "center" }}>
        ...
        <div style={{ position: "relative", maxWidth: 680, margin: "0 auto" }}>
          ...
          <h1 style={{ margin: 0, fontSize: 36, lineHeight: 1.15, fontWeight: 600, letterSpacing: "-0.03em", color: "#fff" }}>{item.title}</h1>
      ...
      <div style={{ background: "#F6F2EA", padding: "48px 40px 64px" }}>
        <div style={{ maxWidth: 720, margin: "0 auto", display: "flex", flexDirection: "column", gap: 24 }}>
          <div style={{ background: "#fff", border: "1px solid rgba(64,50,34,0.13)", borderRadius: 20, padding: "32px 36px" }}>
            ...
            {item.problem && (
              <>
                <h2 style={{ margin: "18px 0 8px", fontSize: 16, fontWeight: 600, color: DARK }}>The Challenge</h2>
                <p style={{ margin: "0 0 20px", fontSize: 14, lineHeight: 1.6, color: "#44444C" }}>{item.problem}</p>
              </>
            )}
            {item.scope && (
              <>
                <h2 style={{ margin: "0 0 8px", fontSize: 16, fontWeight: 600, color: DARK }}>Desired Outcomes</h2>
                <p style={{ margin: "0 0 20px", fontSize: 14, lineHeight: 1.6, color: "#44444C" }}>{item.scope}</p>
              </>
            )}
            {item.support && (
              <>
                <h2 style={{ margin: "0 0 8px", fontSize: 16, fontWeight: 600, color: DARK }}>Support Available</h2>
