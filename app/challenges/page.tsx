...
<div style="position:relative;background:#100D0B;padding:60px 40px 56px;overflow:hidden;text-align:center;">
  ...
  <div style="position:relative;max-width:900px;margin:0 auto;">
    <div style="font-size:12.5px;color:rgba(255,255,255,0.55);margin-bottom:22px;"><a href="${BP}/" style="color:inherit;text-decoration:none;">Home</a> <span style="margin:0 6px;">/</span> <span style="color:rgba(255,255,255,0.7);">Innovation Challenges</span></div>
    ...
    <h1 style="margin:0;font-size:56px;line-height:1.08;font-weight:500;letter-spacing:-0.04em;color:#fff;">Where Baguio&rsquo;s challenges become <span style="color:#F26522;">innovation opportunities.</span></h1>
    ...
    <div style="display:flex;gap:12px;justify-content:center;margin-top:36px;flex-wrap:wrap;">
      <a href="#browse" class="ib-cta-orange" style="display:inline-flex;align-items:center;gap:9px;background:#F26522;color:#fff;font-weight:600;font-size:15.5px;padding:15px 30px;border-radius:9999px;text-decoration:none;box-shadow:0 16px 40px -14px rgba(242,101,34,0.7);">Browse challenges <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.4"><path d="M5 12h14M13 6l6 6-6 6"></path></svg></a>
...
<div style="background:#F6F2EA;padding:64px 40px;">
  <div style="max-width:1080px;margin:0 auto;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
      ...
      <div style="background:#fff;border:1px solid rgba(64,50,34,0.13);border-radius:20px;overflow:hidden;">
        <div style="position:relative;height:180px;background:linear-gradient(135deg,${col.gradA},${col.gradB});overflow:hidden;">
          ...
          <div style="position:absolute;left:22px;right:22px;bottom:18px;">
            <div style="font-size:22px;font-weight:600;letter-spacing:-0.01em;color:#fff;margin-bottom:4px;">${col.heading}</div>
            <div style="font-size:13.5px;color:rgba(255,255,255,0.82);line-height:1.4;">${col.subheading}</div>
          </div>
        </div>
        <div style="padding:26px 28px 28px;">
          <p style="margin:0 0 22px;font-size:14px;line-height:1.6;color:#5A544B;">${col.description}</p>
          <div style="position:relative;">
            <div style="position:absolute;left:13px;top:13px;bottom:13px;width:1px;background:rgba(64,50,34,0.13);"></div>
            ${col.steps.map((step, i) => `
            <div style="position:relative;display:flex;gap:14px;align-items:flex-start;">
              <span style="position:relative;z-index:1;flex-shrink:0;width:26px;height:26px;border-radius:9999px;background:${col.accentSoft};color:${col.accent};font-weight:600;font-size:12.5px;display:flex;align-items:center;justify-content:center;">${i + 1}</span>
              <div style="flex:1;min-width:0;padding-bottom:${i < col.steps.length - 1 ? "16px" : "0"};margin-bottom:${i < col.steps.length - 1 ? "16px" : "0"};${i < col.steps.length - 1 ? "border-bottom:1px solid rgba(64,50,34,0.11);" : ""}">
                <div style="font-size:13.5px;font-weight:600;color:#1A1714;margin-bottom:3px;">${step[0]}</div>
                <div style="font-size:13.5px;color:#5A544B;line-height:1.5;">${step[1]}</div>
              </div>
            </div>`).join("")}
          </div>
          <div style="margin-top:22px;display:flex;gap:14px;align-items:flex-start;background:${col.accentSoft};border-radius:14px;padding:16px 18px;">
            ...
            <div>
              <div style="font-size:13.5px;font-weight:600;color:${col.accent};margin-bottom:2px;">${col.calloutTitle}</div>
              <div style="font-size:13px;color:#5A544B;line-height:1.45;">${col.calloutDesc}</div>
            </div>
          </div>
        </div>
      </div>`).join("")}
...
<!-- WHO CAN PARTICIPATE -->
<div style="background:#131110;padding:68px 40px;position:relative;overflow:hidden;">
...
<div style="background:#F6F2EA;padding:72px 40px;">
  <div style="max-width:1080px;margin:0 auto;display:grid;grid-template-columns:1fr 1fr;gap:20px;">
    <div style="background:#1A1714;border-radius:24px;padding:40px;position:relative;overflow:hidden;">
      ...
      <div style="position:relative;"><h3 style="margin:0 0 10px;font-size:26px;font-weight:600;color:#fff;letter-spacing:-0.02em;">Have a problem to solve?</h3><p style="margin:0 0 24px;font-size:15px;line-height:1.55;color:rgba(255,255,255,0.62);">Post a challenge and tap into Baguio&rsquo;s full innovation community.</p><a href="${BP}/challenges/post" style="display:inline-flex;align-items:center;gap:8px;background:#fff;color:#1A1714;font-weight:600;font-size:15px;padding:14px 26px;border-radius:9999px;text-decoration:none;">Post a challenge <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#1A1714" stroke-width="2.3"><path d="M5 12h14M13 6l6 6-6 6"></path></svg></a></div>
    </div>
    <div style="background:linear-gradient(135deg,#F26522 0%,#E14E12 60%,#C8410C 100%);border-radius:24px;padding:40px;position:relative;overflow:hidden;box-shadow:0 24px 48px -20px rgba(226,78,18,0.5);">
      ...
      <div style="position:relative;"><h3 style="margin:0 0 10px;font-size:26px;font-weight:600;color:#fff;letter-spacing:-0.02em;">Ready to build a solution?</h3><p style="margin:0 0 24px;font-size:15px;line-height:1.55;color:rgba(255,255,255,0.9);">Register as a solver and start submitting to open challenges today.</p><a href="${BP}/signup" style="display:inline-flex;align-items:center;gap:8px;background:#100D0B;color:#fff;font-weight:600;font-size:15px;padding:14px 26px;border-radius:9999px;text-decoration:none;">Become a solver <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.3"><path d="M5 12h14M13 6l6 6-6 6"></path></svg></a></div>
    </div>
  </div>
</div>
