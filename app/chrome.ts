// Shared navbar/footer HTML for every public page, rendered via
// dangerouslySetInnerHTML (same approach as app/dashboard/chrome.ts for the
// dashboard). Centralized here so a nav/footer change lands once instead of
// being hand-applied across every page.tsx that previously inlined its own
// copy of this markup.
...
/**
 * @param active One of NAV_LINKS' `href` values (e.g. "/challenges") to
 * underline as the current page. Omit on pages with no matching nav item
 * (home, contact, dashboard, auth pages, etc).
 */
export function navBarHtml(active?: string) {
  const links = NAV_LINKS.map(
    (n) =>
      `<a href="${BP}${n.href}" class="ib-navlink"${
        active === n.href ? ' style="color:#fff;border-bottom:2px solid #F26522;padding-bottom:3px;"' : ""
      }>${n.label}</a>`
  ).join("\n      ");
